...
class AppStateModel extends foundation.ChangeNotifier {
  ...
  List<Product> _availableProducts = []; // Inicializar como lista vacía
  ...
  double get subtotalCost {
    return _productsInCart.keys.map((id) {
      ...
      return getProductById(id).price * (_productsInCart[id] ?? 0); // Manejo de nulos
  ...
  double get shippingCost {
    return _shippingCostPerItem *
        _productsInCart.values.fold(0.0, (accumulator, itemCount) {
          return accumulator + (itemCount ?? 0); // Manejo de nulos
        });
  }
  ...
  void addProductToCart(int productId) {
    if (!_productsInCart.containsKey(productId)) {
      ...
      _productsInCart[productId] = (_productsInCart[productId] ?? 0) + 1; // Manejo de nulos
    }
  ...
  void removeItemFromCart(int productId) {
    if (_productsInCart.containsKey(productId)) {
      if ((_productsInCart[productId] ?? 0) == 1) {
